...
# Streamlit app setup
st.title('Toronto Bike Share Station Status') # Set the title for the app on top
st.markdown('This dashboard tracks bike availability at each Bike Share Station in Toronto.') # Add a description under the title

# Fetch data for intial visulization
data_df = query_station_status(station_url) # Gets station status
latlon_df = get_station_latlon(latlon_url) # Gets station lat/lon information
...
with st.sidebar:
    ...
    if bike_method == "Rent":
        input_bike_modes = st.multiselect("What kind of bikes are you looking to rent?", ["E-bike", "Mechanical"]) # Multi-select box for bike types
...
# Initial Map Visualization Logic
